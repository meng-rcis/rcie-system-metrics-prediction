...
from keras.layers import Dense, Conv1D, GlobalAveragePooling1D, LayerNormalization
from pconstant.models_id import TCN as TCN_ID
from interface import IBaseModel
from putils.formatter import create_sequences


class TCN(IBaseModel):
    def __init__(self):
        self.dataset = None
    ...
    def TrainModel(self, config: dict) -> Any:
        ...
        X, y = create_sequences(self.scaled_training_dataset, n_past, steps)
        # TCN Model
        model = Sequential(
            [
                Conv1D(
                ...
                LayerNormalization(),
                Conv1D(filters=64, kernel_size=10, dilation_rate=2, activation="relu"),
                LayerNormalization(),
