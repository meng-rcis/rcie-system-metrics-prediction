...
class CNN( IModel ):
    ...
    def ConfigModel(
        ...
        self.testing_dataset = self.dataset.iloc[end_index:end_index+prediction_steps]
        ...
        Xy = self.series_to_supervised(data=feature_values, n_in=self.n_in, n_out=self.n_out)
        Xy_test = self.series_to_supervised(data=feature_values_test, n_in=self.n_in, n_out=self.n_out)
    ...
    def TrainModel(self, config):
        ...
        self.model = model.fit(x=self.X, y=self.y, epochs=1000, batch_size=32, verbose=1)
    ...
    def define_model(self, n_in: int, n_out: int):
        ...
        model.add(keras.layers.Dense(n_out, activation='linear'))  # linear activation for regression problems
